...
resource "docker_container" "src_db" {
  ...
  ports { internal = 5432 external = var.src_host_port }
  mounts { target = "/var/lib/postgresql/data" type = "volume" source = docker_volume.src_db_data.name }
  mounts { target = "/docker-entrypoint-initdb.d" type = "bind" source = "${path.module}/db-init" read_only = true }
  restart = "always"
}
...
resource "docker_container" "dst_db" {
  ...
  ports { internal = 5432 external = var.dst_host_port }
  mounts { target = "/var/lib/postgresql/data" type = "volume" source = docker_volume.dst_db_data.name }
  mounts { target = "/docker-entrypoint-initdb.d" type = "bind" source = "${path.module}/db-init" read_only = true }
  restart = "always"
}
...
resource "docker_container" "api" {
  ...
  ports { internal = 8000 external = var.api_port }
  restart = "always"
}
